...
// tool's rectangle derivation (clamp to the Building bounds, take the whole
...
namespace
{
	...
	// to the Building bounds and taken whole - no shrink-to-free sweep - and
	...
	PaintPreview previewBackgroundPaint(core::Building const& building, uint32_t layerIndex,
		int anchorX, int anchorY, int endX, int endY)
	{
		PaintPreview preview;
		auto const clampedEndX = std::clamp(endX, 0, (int)building.getCellsWide() - 1);
		auto const clampedEndY = std::clamp(endY, 0, (int)building.getDecksHigh() - 1);
		...
		preview.valid = building.canAddBackground(layerIndex, preview.y, preview.x,
	...
	uint32_t releaseBackgroundPaint(core::Building& building, uint32_t layerIndex,
		int anchorX, int anchorY, int endX, int endY)
	{
		auto const preview = previewBackgroundPaint(building, layerIndex, anchorX, anchorY,
			endX, endY);
		if (!preview.valid)
			throw std::runtime_error("Release path ran on an invalid preview: " + preview.diagnostic);
		return building.addBackground(layerIndex, preview.y, preview.x, preview.cellsWide,
			preview.decksHigh);
	}
...
{
	core::Building building("Paint empty", 12, 4);
	building.addRoom("Front room", 0, 0, 0, 12, 4);
	...
	auto const index = releaseBackgroundPaint(building, 1, 5, 2, 0, 0);
	...
	footprintIsStamped(building, 1, index, 0, 0, 6, 3);
}
...
{
	core::Building building("Paint over", 12, 4);
	building.addLayer();
	building.addRoom("Obstacle", 1, 1, 4, 2, 2);
	building.finishBuild();
	building.pauseSimulation();

	// A 6x3 that swallows the Room's cells on the same Layer.
	auto const preview = previewBackgroundPaint(building, 1, 0, 0, 5, 2);
	...
	require(throws([&] { building.addBackground(1, 0, 0, 6, 3); }),
	...
	require(previewBackgroundPaint(building, 1, 0, 3, 3, 3).valid,
	...
	require(previewBackgroundPaint(building, 2, 0, 0, 5, 2).valid,
		"A free Layer refused the same dragged block");
}

// The minimum paint is 1x1; the maximum is capped by the Building bounds,
// which the preview clamps to rather than running past.
void thePaintIsSizedBetweenOneByOneAndTheBuildingBounds()
{
	core::Building building("Paint sizing", 12, 4);
	building.addLayer();
	building.finishBuild();
	building.pauseSimulation();

	auto const single = previewBackgroundPaint(building, 1, 3, 2, 3, 2);
	...
	auto const singleIndex = releaseBackgroundPaint(building, 1, 3, 2, 3, 2);
	require(backgroundIn(building, singleIndex)->getCellsWide() == 1
		&& backgroundIn(building, singleIndex)->getDecksHigh() == 1,
		"The 1x1 Background was not created one cell to a block");

	// Dragging past the edge clamps into the Building instead of refusing.
	auto const clamped = previewBackgroundPaint(building, 2, 0, 0, 99, 99);
	require(clamped.valid, "A drag past the Building bounds was refused instead of clamped");
	require(clamped.cellsWide == 12 && clamped.decksHigh == 4,
		"The clamped drag did not fill the Building bounds");
...
{
	core::Building building("Paint layer targeting", 12, 3);
	building.addLayer();
	building.addRoom("Front room", 0, 0, 0, 6, 3);
	building.addRoom("Back room", 2, 0, 0, 6, 3);
	building.finishBuild();
	building.pauseSimulation();
	...
	auto const middle = previewBackgroundPaint(building, 1, 0, 0, 5, 2);
	require(middle.valid,
		("Occupancy on neighbouring Layers refused a Background on the drag's Layer: "
			+ middle.diagnostic).c_str());
	auto const middleIndex = releaseBackgroundPaint(building, 1, 0, 0, 5, 2);
	require(backgroundIn(building, middleIndex)->getLayerIndex() == 1,
		"The Background did not land on the drag's Layer");
	footprintIsStamped(building, 1, middleIndex, 0, 0, 6, 3);
	auto const& constBuilding = std::as_const(building);
	require(constBuilding.getLayer(0)->getCellDefinition(0, 0).sectorIndex != middleIndex,
		"The Background leaked onto the Layer in front");
	require(constBuilding.getLayer(2)->getCellDefinition(0, 0).sectorIndex != middleIndex,
		"The Background leaked onto the Layer behind");

	// A Background is not a Transit: the front-most Layer takes it with no gate.
	require(previewBackgroundPaint(building, 0, 6, 0, 11, 2).valid,
		"The front-most Layer refused a Background paint");
	releaseBackgroundPaint(building, 0, 6, 0, 11, 2);

	// And so does the back-most Layer.
	require(previewBackgroundPaint(building, 2, 6, 0, 11, 2).valid,
		"The back-most Layer refused a Background paint");
	releaseBackgroundPaint(building, 2, 6, 0, 11, 2);

	building.finishBuild();
	require(building.getNumSectors() == 5,
...
void runBackgroundPaintSmokeChecks()
{
	...
	thePaintIsSizedBetweenOneByOneAndTheBuildingBounds();
