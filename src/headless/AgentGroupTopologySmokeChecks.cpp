...
//
// Almost every structural edit in the Building is a reset and replay: the live
// world is torn down with resetForDeserialization(), the authored construction
// records are replayed against the empty Building, and the Agents that were
...
//
// Every check here drives a real edit through the public Building API and reads
...
// rather than from the Building, so a member that survives but stops counting -
...
// because an assignment that only survives in the live Building would be lost
...
namespace
{
	...
	{
		auto loaded = std::make_shared<core::Building>("Reopened Building", 1, 1);
		core::SerializationWorkData workData;
	...
	Crew authorCrew(core::Building& building, uint32_t frontRoom, uint32_t backRoom)
	{
		Crew crew;
		crew.crew = building.addAgentGroup("Crew");
		crew.gang = building.addAgentGroup("Gang");
		crew.crewFront = building.createAgent("Front desk", frontRoom, 0, 1.0f);
		crew.ungrouped = building.createAgent("Freelance", frontRoom, 0, 2.0f);
		crew.crewBack = building.createAgent("Back office", backRoom, 0, 1.0f);
		crew.gangBack = building.createAgent("Back guard", backRoom, 0, 2.0f);
		assign(building, crew.crewFront, crew.crew);
		assign(building, crew.crewBack, crew.crew);
		assign(building, crew.gangBack, crew.gang);
		return crew;
	}
	...
	// Nothing an Agent carries may name a group this Building does not own. A
	// Building in that state cannot even be saved and read back - the file
	...
	// saved.
	void requireNoDanglingAssignment(core::Building const& building, std::string const& when)
	{
		for (auto const* agent : allAgents(building))
		{
			auto const group = agent->getAgentGroupId();
			if (!group) continue;
			auto const lookup = building.lookupAgentGroup(group);
			require(lookup.entity != nullptr,
				"An Agent carries an Agent group this Building does not own " + when + ": "
	...
	// the Building, so a group whose count drifted from its members fails here
	// even when every individual assignment survived.
	void expectAssignments(core::Building const& building,
		...
		std::string const& when)
	{
		...
		{
			auto const actual = building.getAgentGroup(agent);
		...
		for (auto const& group : groups)
		{
			...
			auto const actual = building.getAgentGroupMemberCount(group);
		...
		requireNoDanglingAssignment(building, when);
	}

	void requireGone(core::Building const& building, core::AgentId agent, std::string const& when)
	{
		require(building.lookupAgent(agent).entity == nullptr,
			"An Agent the edit was meant to remove is still here " + when + ": " + text(agent));
	}

	void requireHere(core::Building const& building, core::AgentId agent, std::string const& when)
	{
		require(building.lookupAgent(agent).entity != nullptr,
	...
	// the whole Building from its records with no Sector moved, which is the
	...
	{
		core::Building building("Plain replay", 12, 3);
		while (building.getLayerCount() < 2) building.addLayer();
		auto const frontRoom = building.addRoom("Landing A", 0, 0, 0, 12, 1);
		auto const backRoom = building.addRoom("Landing B", 0, 1, 0, 12, 1);
		building.addRoom("Landing C", 0, 2, 0, 12, 1);
		auto const lift = building.addLift(1, 0, 2, 1, 3);
		building.finishBuild();

		auto const crew = authorCrew(building, frontRoom, backRoom);
		...
		auto const plan = building.planResizeLift(liftSector, 2, 0, 1, 3);
		require(plan.valid, "The unchanged-topology Lift rebuild plan was refused: " + plan.diagnostic);
		building.applyLiftEdit(plan);

		requireHere(building, crew.crewFront, "after a plain construction-record replay");
		requireHere(building, crew.crewBack, "after a plain construction-record replay");
		requireHere(building, crew.gangBack, "after a plain construction-record replay");
		requireHere(building, crew.ungrouped, "after a plain construction-record replay");
		expectAssignments(building, expectedOf(crew), bothGroups(crew),
	...
	{
		core::Building building("Layer deletion", 12, 3);
		while (building.getLayerCount() < 3) building.addLayer();
		auto const frontRoom = building.addRoom("Front room", 0, 0, 0, 6, 1);
		auto const backRoom = building.addRoom("Back room", 1, 0, 0, 6, 1);
		auto const doomedRoom = building.addRoom("Top room", 2, 0, 0, 6, 1);
		building.finishBuild();

		auto const crew = authorCrew(building, frontRoom, backRoom);
		auto const crewTop = building.createAgent("Top floor hand", doomedRoom, 0, 1.0f);
		assign(building, crewTop, crew.crew);
		...
		expectAssignments(building, beforeWithTop, bothGroups(crew),
			"before a Layer deletion");
		require(building.getAgentGroupMemberCount(crew.crew) == 3,
			"The fixture did not put three Agents in the Crew group");

		building.pauseSimulation();
		auto const plan = building.planDeleteLayer(2);
		...
		requireGone(building, crewTop, "after a Layer deletion");
		requireHere(building, crew.crewFront, "after a Layer deletion");
		requireHere(building, crew.crewBack, "after a Layer deletion");
		// The deleted Agent is not counted: the group's live count is its
		// surviving members, not its former ones.
		expectAssignments(building, before, bothGroups(crew), "after a Layer deletion");
		require(building.getAgentGroupMemberCount(crew.crew) == 2,
	...
	{
		core::Building building("Door removal", 12, 3);
		while (building.getLayerCount() < 2) building.addLayer();
		auto const frontRoom = building.addRoom("Front room", 0, 0, 0, 8, 1);
		...
		requireHere(building, crew.crewFront, "after a Door removal");
		requireHere(building, crew.crewBack, "after a Door removal");
		requireHere(building, crew.gangBack, "after a Door removal");
		requireHere(building, crew.ungrouped, "after a Door removal");
		expectAssignments(building, expectedOf(crew), bothGroups(crew),
	...
	{
		core::Building building("Window removal", 12, 3);
		while (building.getLayerCount() < 3) building.addLayer();
		auto const frontRoom = building.addRoom("Front room", 0, 0, 0, 8, 1);
		auto const backRoom = building.addRoom("Back room", 1, 0, 0, 8, 1);
		...
		requireHere(building, crew.crewFront, "after a Window removal");
		requireHere(building, crew.crewBack, "after a Window removal");
		requireHere(building, crew.gangBack, "after a Window removal");
		requireHere(building, crew.ungrouped, "after a Window removal");
		expectAssignments(building, expectedOf(crew), bothGroups(crew),
	...
	{
		core::Building building("Object movement", 12, 4);
		while (building.getLayerCount() < 2) building.addLayer();
		auto const frontRoom = building.addRoom("Front room", 0, 0, 0, 4, 3);
		auto const backRoom = building.addRoom("Back room", 1, 0, 0, 4, 3);
		auto const walkway = building.addSectorWalkway(frontRoom, 1, 1);
		building.finishBuild();
		...
		building.pauseSimulation();
		auto const plan = building.planMoveSectorObject(frontRoom, walkway.index, 2, 1);
		require(plan.valid, "The object move plan was refused: " + plan.diagnostic);
		auto const moved = building.applyObjectMove(plan);
		...
		requireHere(building, crew.crewFront, "after an object move");
		requireHere(building, crew.crewBack, "after an object move");
		requireHere(building, crew.gangBack, "after an object move");
		requireHere(building, crew.ungrouped, "after an object move");
		expectAssignments(building, expectedOf(crew), bothGroups(crew),
	...
	{
		core::Building building("Location editing", 12, 3);
		while (building.getLayerCount() < 2) building.addLayer();
		auto const frontRoom = building.addRoom("Front room", 0, 0, 0, 4, 1);
		auto const backRoom = building.addRoom("Back room", 1, 0, 0, 4, 1);
		building.finishBuild();
		...
		building.pauseSimulation();
		auto const plan = building.planResizeLocation(frontRoom, 6, 0, 4, 1);
		...
		requireHere(building, crew.crewFront, "after a Location move");
		requireHere(building, crew.crewBack, "after a Location move");
		requireHere(building, crew.gangBack, "after a Location move");
		requireHere(building, crew.ungrouped, "after a Location move");
		...
		auto const movedRoom = building.getSectorAtPosition(0, 7.5f, 0.5f);
	...
	// replays every other Sector and every Agent in the Building.
	void backgroundEditingKeepsEveryAssignment()
	{
		core::Building building("Background editing", 12, 3);
		while (building.getLayerCount() < 3) building.addLayer();
		auto const frontRoom = building.addRoom("Front room", 0, 0, 0, 4, 1);
		auto const backRoom = building.addRoom("Back room", 1, 0, 0, 4, 1);
		auto const background = building.addBackground(2, 0, 0, 4, 1, { 10, 20, 30 });
		building.finishBuild();
		...
		building.pauseSimulation();
		auto const plan = building.planResizeBackground(background, 0, 0, 6, 1);
		require(plan.valid, "The Background resize plan was refused: " + plan.diagnostic);
		building.applyBackgroundEdit(plan);

		requireHere(building, crew.crewFront, "after a Background edit");
		requireHere(building, crew.crewBack, "after a Background edit");
		requireHere(building, crew.gangBack, "after a Background edit");
		requireHere(building, crew.ungrouped, "after a Background edit");
		expectAssignments(building, expectedOf(crew), bothGroups(crew),
			"after a Background edit");
	}

	// The assignments the edit left behind have to survive the file: the saved
	// document carries them, and a reopen agrees with the live Building about
	...
	// to agree with the live Building about every assignment.
	void anEditSurvivesUndoAndRedo()
	{
		...
		requireHere(*building, crew.crewFront, "after undoing the edit");
		requireHere(*building, crew.ungrouped, "after undoing the edit");
		expectAssignments(*building, expectedOf(crew), bothGroups(crew),
			"after undoing the topology edit");

		restoreDocument(building, true);
		requireHere(*building, crew.crewFront, "after redoing the edit");
		requireHere(*building, crew.ungrouped, "after redoing the edit");
		expectAssignments(*building, expectedOf(crew), bothGroups(crew),
