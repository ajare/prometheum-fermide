...
namespace core
{
	...
	public:
		...
		LadderVertex(uint32_t id, std::shared_ptr<Sector> sector, std::shared_ptr<Ladder> ladder, float xOffset, float yOffset, int level);

		LadderVertex(std::shared_ptr<Sector> sector, std::shared_ptr<Ladder> ladder, float xOffset, float yOffset, int level);
