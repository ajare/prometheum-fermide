...
namespace
{
	...
	std::vector<uint32_t> usableStopOffsets(std::vector<core::Building::LiftLandingRow> const& rows)
	{
	...
	LiftPreview previewLift(core::Building const& building, uint32_t shaftLayer, uint32_t y,
		uint32_t x, uint32_t cellsWide, uint32_t decksHigh)
	{
		LiftPreview preview;
		for (auto const& row : building.getLiftLandingRows(shaftLayer, y, x, cellsWide, decksHigh))
		{
	...
	std::set<std::string> authoredNames(core::Building const& building)
	{
	...
	std::string nextRoomName(core::Building const& building)
	{
		auto const names = authoredNames(building);
	...
	// Authors a four-Layer Building whose Lift landings sit on Layer 1: two
	...
	void authorLiftLandings(core::Building& building)
	{
		while (building.getLayerCount() < 4) building.addLayer();
		for (uint32_t row = 0; row < building.getDecksHigh(); ++row)
			building.addCorridor(0, row, 0, 40, 1);
		building.addCorridor(1, 1, 8, 12, 1);
		building.addCorridor(1, 3, 8, 12, 1);
	}

	uint32_t cellSector(core::Building const& building, uint32_t layer, uint32_t x, uint32_t y)
	{
		return building.getLayer(layer)->getCellDefinition(x, y).sectorIndex;
	}
}
...
void liftLandingsComeFromTheLayerInFront()
{
	...
	core::Building building("Lift landing layers", 40, 6);
	authorLiftLandings(building);

	auto const rows = building.getLiftLandingRows(2, 0, 12, 2, 5);
	...
	require(usableStopOffsets(building.getLiftLandingRows(1, 0, 12, 2, 5)).size() == 5,
		"The Layer directly in front of Layer 1 was not the carpeted Layer 0");
	require(previewLift(building, 2, 0, 12, 2, 5).stops == 2,
	...
	auto const created = building.addLift(2, 0, 12, 2, 5);
	...
	require(building.getLiftLandingRows(0, 0, 12, 2, 5).empty(),
		"The front-most Layer reported Lift landings");
	require(previewLift(building, 0, 0, 12, 2, 5).stops == 0,
...
void liftLandingsReportObstructionAndCallButtonSpace()
{
	...
	require(previewLift(building, 2, 0, 12, 2, 5).stops == 1,
	...
	building.addCorridor(1, 1, 24, 2, 1);
	building.addCorridor(1, 3, 24, 2, 1);
	auto const spanning = building.getLiftLandingRows(2, 0, 24, 2, 5);
	...
	require(previewLift(building, 2, 0, 24, 2, 5).callButtonBlocked,
...
void windowDefinitionsAreFoundOnTheWindowLayer()
{
	...
	core::Building building("Window copy layers", 12, 3);
	while (building.getLayerCount() < 4) building.addLayer();
	building.addCorridor(0, 0, 0, 12, 1);
	building.addRoom("Store", 1, 0, 0, 12, 1);
	building.addRoom("Depot", 2, 0, 0, 12, 1);
	building.addRoom("Back Room", 3, 0, 0, 12, 1);

	auto const created = building.addSectorWindow(2, 0, 5, 2, 1, { true });
	...
	require(building.getSectorWindowOptions(2, 0, 5, 2, 1, options),
	...
	require(!building.getSectorWindowOptions(0, 0, 5, 2, 1, options)
		&& !building.getSectorWindowOptions(1, 0, 5, 2, 1, options),
...
void everyLayerEnumeratesItsSectorsAndAgents()
{
	...
	core::Building building("Deep enumeration", 16, 2);
	while (building.getLayerCount() < 4) building.addLayer();
	auto const corridor = building.getSector(building.addCorridor(0, 0, 0, 16, 1));
	building.addRoom("Room 1", 1, 0, 0, 16, 1);
	building.addRoom("Depot", 2, 0, 0, 16, 1);
	building.addRoom("Room 2", 3, 0, 0, 16, 1);

	for (uint32_t layer = 0; layer < building.getLayerCount(); ++layer)
	{
		auto const sectors = building.getSectors(layer);
		require(!sectors.empty(), "A Layer of the Building enumerated no Sectors");
		bool sawDepot = false;
	...
	building.createAgent("Front agent", corridor->getIndex(), 0, 0.5f);
	auto const depot = std::dynamic_pointer_cast<const core::Location>(
		building.getSector(cellSector(building, 2, 0, 0)));
	require(depot && depot->getName() == "Depot", "The Depot could not be resolved on Layer 2");
	building.createAgent("Deep agent", depot->getIndex(), 0, 0.5f);
	...
	require(nextRoomName(building) == "Room 3",
...
void corridorsAreAuthorableOnEveryLayer()
{
	...
	core::Building building("Corridor layers", 24, 3);
	while (building.getLayerCount() < 4) building.addLayer();
	for (uint32_t layer = 0; layer < building.getLayerCount(); ++layer)
	{
		auto const index = building.addCorridor(layer, 0, layer * 5, 4, 1);
		auto const sector = std::dynamic_pointer_cast<const core::Location>(building.getSector(index));
