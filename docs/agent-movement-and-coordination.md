...
Passengers are attached to the moving vehicle while onboard. Open platform lifts use a virtual boundary governed by the same transport policy instead of relying on a physical door. Each platform stop has a physical waiting lane: callers retain logical ticket priority, reserve separated queue positions after operating the landing control, and must reach the assigned position before boarding admission. Crossing the platform boundary begins boarding without teleporting the passenger. The car remains stationary and the crossing stays reserved until the passenger's full width is within the platform bounds; occupancy is established at that point, after which ordinary locomotion may continue toward the reserved onboard position while vehicle motion is applied independently. Path following applies the general skippable-vertex rule on final exit: after looking through coincident topology-only nodes, an intermediate waypoint is skipped only when it and the following physical vertex share the agent's layer and height, lie on opposite horizontal sides of the agent, and require no action at the intermediate waypoint. Contiguous Platform Lift graph legs form one transport journey to the final requested stop; intermediate graph floors do not create stop demand, and the shared LOOK scheduler passes them unless another caller or passenger requested them.
